...
class TestTinybird(TestCase):
    ...
    def test_run_query(self, get_request):
        ...
        data, error = query_runner.run_query(
            "SELECT * FROM test_datasource LIMIT 1", None
        )
    ...
    def _mock_tinybird_schema_requests(self, endpoint, **kwargs):
        ...
        return Mock(
            status_code=200, text=json.dumps(response), json=Mock(return_value=response)
        )

    def _build_query_runner(self):
        return Tinybird(
            {"url": "https://api.tinybird.co", "token": "p.test.token", "timeout": 60}
        )
